...
cluster_medoid <- function(
  ...
  result <- tibble::tibble("n" = n_min : n_max) %>%
    mutate(
      ...
      "cluster_fit" = purrr::map(n, possibly(~cluster::pam(df_temp, .x), otherwise = NA)),
      "cluster_seed" = purrr::map2(cluster_fit, cluster_name, possibly(~pluck(.x, "clustering") %>% bind_cols(id_temp, .) %>% set_names(c("id", .y)) %>% suppressMessages(), otherwise = NA)),
