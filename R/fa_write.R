...
#' @export
fa_write <- function(
    fa_analysis_object, file_name = "Factor Analysis",  where = NULL
){
  ...
  append_sheet <- function(wb, fa_table, sheet_name, footer = NULL, clean_max = .25, row_start = 4, col_start = 2){
  ...
  iwalk(
    ...
    ~ append_sheet(wb, fa_table = .x, sheet_name = .y, footer = glue("Rotation: {rotation}"))
  )

  saveWorkbook(wb, glue("{where}/{file_name}.xlsx"), overwrite = TRUE)
}
